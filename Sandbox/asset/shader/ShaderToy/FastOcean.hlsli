...
//        res += sin(a);
//        dres += cos(a) * mul(vec2(1., 2. * cos(2. * p.y)), j);
//        j *= m2 * f;
...
//void FastOcean(out vec4 fragColor, in vec2 fragCoord)
//{
...
//            col1 += pow(max(dot(li, nor), 0.0), 5.0) * vec3(.8,.8,.8);
