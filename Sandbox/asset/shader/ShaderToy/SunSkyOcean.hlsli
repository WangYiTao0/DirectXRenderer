...
float noise(in vec3 v)
{
    ...
    vec2 rg = iChannel1.SampleLevel(splr, (uv + .5) / 256., 0.).yx;
    return mix(rg.x, rg.y, f.z);
}

vec3 random3f(vec3 p)
{
    return iChannel1.SampleLevel(splr, (p.xy + vec2(3.0, 1.0) * p.z + 0.5) / 256.0, 0.0).xyz;
}
    ...
    if (rd.y > -0.1)
    {
        ...
        if (domeT > -0.5)
        {
            ...
            for (int i = 0; i < 4; ++i)
            {
                ...
                // diff lighting
                vec4 clr = vec4(vec3(0.3,0.3,0.3), 0.0) + vec4(haloclr2 * dif * 5.0, d1);
...
vec3 renderSky(vec3 rd)
{
    // sun 
    float sundot = clamp(dot(sundir, rd), 0., 1.);
    vec3 core = vec3(1.0,1.0,1.0) * pow(sundot, 250.0);
...
static const float WaveTimeMax = 6.0;
static const vec3 WaveOffset = vec3(2.8, 0.0, 0.0);
...
static const float d1 = 4.0;
static const float d2 = 2.0;
static const float d3 = 3.0;
static const float totalPhaseTime = d1 + d2 + d3;
...
static const float kSeaHeight = 1.0;
vec4 mapWave(vec3 p, out vec2 ph)
{
    ...
    return vec4(df.x, df.y, df.z, ph.x);
}
...
vec4 traceWaterBombVol(vec3 ro, vec3 rd, float depth)
{
    ...
    vec3 sunlgt = normalize(vec3(1.0,1.0,1.0));
    ...
    return vec4(vec3(opacity, opacity, opacity), opacity);
}
...
vec3 renderSunSkyOcean(in vec3 ro, in vec3 rd)
{
    vec3 clr = vec3(0.0,0.0,0.0);
    ...
    if (t > -0.5 && (t < ot || ot < 0.0))
    {
        ...
        if (ws < 0.0 || ws >= 1.0)
        {
            clr = mix(clr, vec3(0.5,0.5,0.5) + haloclr1, tho);
        }
    }
#endif
    ...
    if (t > -0.5 && (t < ot || ot < 0.0))
    {
        ...
        clr = mix(clr, vec3(0.15,0.15,0.15) + haloclr1, min(col4.a, 1.0));
...
mat3 camera(in vec3 pos, in vec3 lookat, float roll)
{
    ...
    vec3 x = vec3(sin(roll), cos(roll), 0.0);
    x = cross(z, x);
    vec3 y = cross(x, z);
    return mat3(x, y, z);
}

void SunSkyOcean(out vec4 fragColor, in vec2 fragCoord)
{
    vec2 q = fragCoord.xy;
    vec2 p = -1.0 + 2.0 * q;
    ...
    vec3 col = renderSunSkyOcean(ro, rd);
