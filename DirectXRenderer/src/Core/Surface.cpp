...
namespace dr
{
	...
	unsigned int dr::Surface::GetBytePitch() const noexcept
	{
		...
		note("[Note] " + note)
	{
		if (hr)
		{
			note = "[Error String] " + Win32Window::Exception::TranslateErrorCode(*hr) + "\n" + note;
		}
	}
		...
		DrException(line, file)
	{
		...
		if (hr)
		{
			note = "[Error String] " + Win32Window::Exception::TranslateErrorCode(*hr) + note;
		}
	}
	...
	const char* Surface::Exception::what() const noexcept
	{
		...
		oss << DrException::what() << std::endl << GetNote();
		whatBuffer = oss.str();
