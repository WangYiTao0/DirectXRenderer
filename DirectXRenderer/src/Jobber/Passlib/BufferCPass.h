...
namespace dr
{
	...
	namespace Rgph
	{
		class BufferCPass : public RenderQueuePass
		{
		public:
			BufferCPass(Graphics& gfx, std::string name, unsigned int fullWidth,
				...
				RenderQueuePass(std::move(name))
			{
				...
				renderTarget = std::make_unique<ShaderInputRenderTarget>(gfx,
					fullWidth / 2, fullHeight / 2, 5);
				AddBind(PixelShader::Resolve(gfx, "BufferC_PS"));
